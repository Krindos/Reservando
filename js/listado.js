...
Listado.prototype.buscarRestaurante = function(id) {
    var resultadoBusqueda=this.restaurantes.find(restaurante=>restaurante.id===id);
...
function filtrar(elementos){
    // var todosLosElementos = [];
    // todosLosElementos.forEach(restaurante => todosLosElementos.push(restaurante.ubicacion));
    
    return elementos.filter((elem, index, self) => index === self.indexOf(elem)).sort();
}

//Obtiene todas las ciudades de los restaurantes sin repetidos
Listado.prototype.filtrarPorCiudad = function() {
    //Array donde se van a ir agregando las ciudades (van a estar repetidas)
    var todasLasCiudades = [];
    todasLasCiudades=this.restaurantes.map(restaurante => restaurante.ubicacion);
    return filtrar(todasLasCiudades);
}

//Obtiene todos los rubros de los restaurantes sin repetidos. Su funcionamiento es similar a obtC()
Listado.prototype.filtrarPorRubro = function() {
    //Array donde se van a ir agregando los rubros (van a estar repetidos)
    var todosLosRubros = [];
    todosLosRubros=this.restaurantes.map(restaurante => restaurante.rubro);
    return filtrar(todosLosRubros);
}
...
Listado.prototype.filtrarPorHorario = function() {
    //Array donde se van a ir agregando los horarios (van a estar repetidos)
    var todosLosHorarios=[];
    todosLosHorarios=this.restaurantes.map(restaurante => restaurante.horarios);
    var horas = [];
    todosLosHorarios.forEach(horasDisponibles => horasDisponibles.forEach(hora=> horas.push(hora)));
    return filtrar(horas);
}
...
//     var reserva=new Reserva(new(2019,11,08,19,13),1,100,null);
// }
